...
short map_element_neighbour_count(const char* const map,
                                  ...
                                  const size_t square_map_length);
short map_remove_elements_with_less_than_four_neighbours(
    char* const map, const size_t square_map_length);
    ...
    for (size_t i = 0; i < square_map_length; i++) {
        for (size_t j = 0; j < square_map_length; j++) {
            ...
            if (map[coord_index(coord, square_map_length)] == '.') continue;
                ...
                map[coord_index(coord, square_map_length)] = '.';
                count++;
                // We move to a diagonal element two rows up and two cols back
                // to revisit any elements that could possibly be removed.
                i = (i - 2 > square_map_length) ? 0 : i - 2;
                j = (j - 2 > square_map_length) ? 0 : j - 2;
            }
        }
    }
...
int main(void) {
    ...
    printf("The number of elements with <4 neighbours removed is %hd.\n",
           map_remove_elements_with_less_than_four_neighbours(map, SQUARE_MAP_LENGTH));

...
long highest_12dig_number_in_line(const char* const line);
...
long highest_12dig_number_in_line(const char* const line) {
    const size_t HIGH_NUM_LEN = 12, line_len = strlen(line);
...
int main(void) {
    ...
    while (true) {
        ...
        highest_number_pair_sum += highest_12dig_number_in_line(line);
        free(line);
    }

    printf("The sum of the highest 12-digit number in each line is %ld.\n",

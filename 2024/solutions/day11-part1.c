...
long stones_after_blink(long stone_number, short curr_depth, short max_depth);
long number_digits_half(const long number, const long number_ndigits,
...
long stones_after_blink(const long stone_number, const short curr_depth,
    ...
    if (stone_number == 0)
        nstones += stones_after_blink(1, curr_depth + 1, max_depth);
        ...
        nstones += stones_after_blink(stone_number * 2024, curr_depth + 1, max_depth);
...
int main(void) {
    ...
    for (size_t i = 0; i < nsplits; i++) {
        total_stones += stones_after_blink(num_list[i], 0, BLINKS);
    }

    printf("The total number of stones after %hd blinks is %ld\n", BLINKS,
           total_stones);

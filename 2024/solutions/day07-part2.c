...
bool is_expression_valid(const long* const numbers, const size_t size,
    ...
    return (
        is_expression_valid(numbers + 1, size - 1, current_result + numbers[0],
                            final_result) ||
        is_expression_valid(numbers + 1, size - 1, current_result * numbers[0],
                            final_result) ||
        is_expression_valid(numbers + 1, size - 1,
                            combine_two_numbers_in_order(current_result, numbers[0]),
                            final_result));
}

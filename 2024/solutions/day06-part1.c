...
enum GuardDirection { NORTH = 1, EAST = 2, SOUTH = 3, WEST = 4 };
...
enum GuardDirection
guard_direction_after_turning_right(const enum GuardDirection guard_direction);

struct Coord
coord_generate_next_coord_with_direction(const struct Coord guard_location,
                                         const enum GuardDirection guard_direction);

bool coord_at_map_boundary(const struct Coord coord, const int square_map_length);

bool map_obstacle_at_coord(const unsigned short* const map, const int square_map_length,
                           struct Coord coord);
void map_mark_coord_as_visited(unsigned short* const map, const int square_map_length,
                               struct Coord guard_location);
int map_get_visited_locations_count(const unsigned short* const map,
                                    const int square_map_length);
void map_print(const unsigned short* const map, const int square_map_length,
               struct Coord guard_location);
...
coord_generate_next_coord_with_direction(const struct Coord guard_location,
    ...
    switch (guard_direction) {
        case NORTH:
            return (struct Coord){.row = guard_location.row - 1,
                                  .col = guard_location.col};
        case EAST:
            return (struct Coord){.row = guard_location.row,
                                  .col = guard_location.col + 1};
        case SOUTH:
            return (struct Coord){.row = guard_location.row + 1,
                                  .col = guard_location.col};
        // WEST
...
bool coord_at_map_boundary(const struct Coord coord, const int square_map_length) {
    return ((coord.row == (square_map_length - 1)) ||
            (coord.col == (square_map_length - 1)) || (coord.row == 0) ||
            (coord.col == 0));
}
...
int main(void) {
    ...
    unsigned short* const map =
        calloc(SQUARE_MAP_LENGTH * SQUARE_MAP_LENGTH, sizeof(unsigned short));
    ...
    while (true) {
        struct Coord new_coord =
            coord_generate_next_coord_with_direction(guard_location, guard_direction);
    ...
    printf("Count of unique visited locations is %d\n",
